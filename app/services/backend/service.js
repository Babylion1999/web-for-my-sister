...
const fs = require('fs');
const MainModel 	= require(__path_schemas + 'service');
...
module.exports = {
    ...
    listItemsFrontend: async(params = null, options = null, limit_options=null,pagination=null, objWhere=null)=>{
        ...
        if(options.task=='items-in-service'){
            return MainModel
            .find({status: 'active'})
    ...
    delete:  async(id)=>{
        await MainModel.findById(id).then((item)=>{
            let path='public/backend/adminlte/images/service/'+item.thumb;
    ...
    deleteMulti: async(cids)=>{
        ...
        if(Array.isArray(cids)){
            ...
            await MainModel.findById(cids.$in).then((item)=>{
                let path='public/adminlte/images/service/'+item.thumb;
                fileHelpers.removeImg(path);        
            });
        }
