...
const ValidateService	= require(__path_validates + `${changeName}`);
...
const pageTitleIndex = 'Service Management';
const pageTitleAdd   = pageTitleIndex + ' - Add';
const pageTitleEdit  = pageTitleIndex + ' - Edit';
const fileHelpers = require(__path_helpers + 'upload');
const uploadThumb = fileHelpers.uploadImg('thumb','backend/adminlte/images/service')
...
router.post('/save', async(req, res, next) => {
	...
	if(errors.length > 0) { 
        if(req.file!=undefined){
            let path='public/adminlte/images/service/'+ req.file.filename;
        ...
        let massage= (taskCurrent=="edit") ? notify.EDIT_SUCCESS : notify.ADD_SUCCESS;
         if(req.file!=undefined){
            ...
            if(taskCurrent==="edit") 
            {
                 let path= 'public/backend/adminlte/images/service/'+ item.image_old;
