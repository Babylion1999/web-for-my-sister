...
const ValidateSlider	= require(__path_validates + `${changeName}`);
...
const uploadThumb = fileHelpers.uploadImg('thumb','backend/adminlte/images/slider')
...
router.post('/save', async(req, res, next) => {
	...
	if(errors.length > 0) { 
        if(req.file!=undefined){
            let path='public/adminlte/images/slider/'+ req.file.filename;
        ...
        let massage= (taskCurrent=="edit") ? notify.EDIT_SUCCESS : notify.ADD_SUCCESS;
         if(req.file!=undefined){
            ...
            if(taskCurrent==="edit") 
            {
                 let path= 'public/backend/adminlte/images/slider/'+ item.image_old;
